              ...
              <div key={index} className="p-4 hover:bg-gray-50">
                <div className="mb-2 flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="mb-1 text-sm font-medium text-primary">{item.company[language]}</div>
                    <div className="mb-2 text-sm">{item.title[language]}</div>
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      <span className="rounded bg-blue-50 px-2 py-1 text-primary">{item.type[language]}</span>
                      <span>{item.date}</span>
                    </div>
                  </div>
                  {item.amount && (
          ...
          <div className="divide-y divide-border">
            {meetingsData.map((item, index) => (
              <div key={index} className="p-4 hover:bg-gray-50">
                <div className="mb-2">
                  <div className="mb-1 text-sm font-medium text-primary">{item.company[language]}</div>
                  <div className="mb-2 text-sm">{item.topic[language]}</div>
                  <div className="flex items-center gap-4 text-xs text-muted-foreground">
                    <span className="rounded bg-purple-50 px-2 py-1 text-purple-700">{item.type[language]}</span>
                    <span>{item.date}</span>
                  </div>
                </div>
              </div>
